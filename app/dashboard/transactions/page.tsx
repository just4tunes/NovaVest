...
function getTransactionDetails(type: TransactionCategory) {
  switch (type) {
    case "profit":
...
function ActivityIcon({ category }: { category: TransactionCategory }) {
  switch (category) {
    case "profit":
...
export default function TransactionsPage() {
  ...
  const loadTransactions = useCallback(async () => {
    try {
      ...
      const depositActivities: Activity[] = deposits.map((deposit) => ({
        id: `deposit-${deposit._id}`,
        title: `${deposit.cryptoAsset} deposit`,
        details: deposit.network,
        amount: deposit.amount,
        reference: deposit.transactionHash,
       ...
       */
      const balanceActivities: Activity[] = transactions
        .filter((transaction) => transaction.type !== "deposit")
        .map((transaction) => ({
          id: `transaction-${transaction._id}`,
          title: transaction.description || "Account transaction",
          details: getTransactionDetails(transaction.type),
      ...
      <header className="dashboard-header">
        <div>
          ...
          <p className="mobile-hide-subtext">
            Review your deposits, profits and account activity.
          </p>
        </div>
      ...
      <section className="dashboard-panel">
        ...
        {loading ? (
          ...
          <div className="admin-empty-state">
            ...
            <p>Your deposits, profits and investments will appear here.</p>
          </div>
        ) : (
                  ...
                  <tr key={activity.id}>
                    ...
                    <td>
                      {activity.balanceAfter !== undefined
                        ? formatMoney(activity.balanceAfter)
                        : "—"}
                    </td>
                  </tr>
                ))}
